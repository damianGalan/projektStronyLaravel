...
<div id="menu">
    <table class="table table-hover" >
        <form action="{{route('Sites.usun')}}" method="post">

            <h1>&nbsp Historia zamówień</h1>
            &nbsp *przycisk "usuń" usuwa ostatnią pozycję<br/><br/>

            <input type="hidden" name="_token" value="{{csrf_token()}}">
        @foreach($zamowienias as $site)
            <tr>
                <td>{{$site->id}}</td>
                <input type="hidden" name="przycisk" value="{{$site->id}}">
                <td>{{$site->osoba}}</td>
                <td>{{$site->adres}}</td>
                <td>{{$site->numer}}</td>
